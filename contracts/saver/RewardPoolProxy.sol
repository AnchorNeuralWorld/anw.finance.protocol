...
import "../util/OwnableStorage.sol";

contract RewardPoolProxy is Proxy{
    ...
    function initialize( address implAddress, bytes memory initData, address storage_ ) public isInitializer{
        ...
        require(_OWNABLE_STORAGE_SLOT == bytes32(uint256(keccak256("anw.finance.reward.proxy.ownablestrage")) - 1));
        require(_INITIALIZE_SLOT == bytes32(uint256(keccak256("anw.finance.reward.proxy.initialize")) - 1));
        ...
        if(initData.length > 0) {
            _functionDelegateCall(implAddress, initData);
        }
