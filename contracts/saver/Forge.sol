...
import "./interfaces/RewardPoolInterface.sol";
...
contract Forge is ForgeInterface, Ownable, ERC20, ReentrancyGuard, ForgeStorage {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    uint constant SECONDS_DAY = 86400;

    constructor() ERC20("ANW Finance", "Forge") {}

    /**
    * Initializing Forge's Variables, If already initialized, it will be reverted.
    * 
    * @param storage_ deployed OnwableStorage's address 
    ...
    */
    function withdrawable( address account, uint index ) public view override returns( uint ){
        SaverStructs.Saver memory s = saver( account, index );
    ...
    */
    function craftingSaver( uint amount, uint startTimestamp, uint count, uint interval, bytes12 referral ) public override returns( bool ){
        ...
        _savers[ msg.sender ].push( SaverStructs.Saver( block.timestamp, startTimestamp, count, interval, 0, 0, 0, 0, 0, 0, block.timestamp, referral ) );
        _transactions[ msg.sender ][ index ].push( SaverStructs.Transaction( true, block.timestamp, 0 ) );
        _count++;
    ...
    function addDeposit( uint index, uint amount ) public nonReentrant override returns( bool ){
...

        {
            ...
            if( _variables.reward() != address(0) ) {
                ...
                RewardPoolInterface( _variables.reward() ).staking( address(this), mint, msg.sender );
            }
        }

        {
            ...
            if( block.timestamp.sub( transactions(msg.sender, i )[ lastIndex ].timestamp ) < SECONDS_DAY ){
                ...
                _transactions[msg.sender][ index ].push( SaverStructs.Transaction( true, block.timestamp, amount ) );
            }
    ...
    */
    function withdraw( uint index, uint amountPlp ) public nonReentrant override returns( bool ){
        SaverStructs.Saver memory s = saver( msg.sender, index );
...

        {
            ...
            if( _variables.reward() != address(0) ) RewardPoolInterface( _variables.reward() ).unstaking(address(this), amount, msg.sender );
    ...
    */
    function terminateSaver( uint index ) public nonReentrant override returns( bool ){
        ...
        /* for pLP token */
        {
            ...
            if( _variables.reward() != address(0) ) RewardPoolInterface( _variables.reward() ).unstaking(address(this), lp, msg.sender );
    ...
    * @return model address.
    */
    function saver( address account, uint index ) public view override returns( SaverStructs.Saver memory ){ return _savers[account][index]; }
    ...
    * @return deposit & withdrawn histories
    */
    function transactions( address account, uint index ) public view override returns ( SaverStructs.Transaction [] memory ){ return _transactions[account][index]; }
