...
library Score {
    ...
    function _getDepositTransactions( uint createTimestamp, uint deposit, SaverStructs.Transaction [] memory transactions ) private pure returns( uint depositCount, uint [] memory xAxis, uint [] memory yAxis ){
        depositCount = 0;
    ...
    function calculate( uint createTimestamp, uint startTimestamp, SaverStructs.Transaction [] memory transactions, uint count, uint interval, uint decimals ) public pure returns ( uint ){
